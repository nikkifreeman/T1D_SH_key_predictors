...
collectVariables <- function(dataTables){
  ...
  sampleResultsDF <- dataTables$BSampleResults.txt %>%
    ...
    filter(Analyte %in% c("CPEP", "CREA-S", "HBA1C")) 
  dim(sampleResultsDF)
  ...
  # Collect the Blood glucose attitudes scale variables
  bgAttitudeScaleDF <- dataTables$BBGAttitudeScale.txt %>% 
    dplyr::select(PtID, BBGAttitudeScaleNotDone, 
           DealHypoEp, UndertreatHypo, HighBGDamage, 
           FreqHypoDamage, DangersHighBG)
  dim(bgAttitudeScaleDF)
  bgAttitudeScaleDF <- left_join(caseControl, bgAttitudeScaleDF, by = "PtID")
  dim(bgAttitudeScaleDF)
  ...
  dataTables$BHypoFearSurvey.txt %>%
  ...
  analysisData_raw <- demographicsDiabDF %>% 
    left_join(bgAttitudeScaleDF, by = c("PtID", "BCaseControlStatus", "exclude")) %>% 
